...
This function follows the implementation of the stat_glmnet function,
but modifies it to focus on the coefficients of the interaction terms
with the treatment. A first version of this filter was suggested in
Sechidis et al. (2021), but with this function we also allow the user to
adjust to variables, or to fix some variables as prognostic in the
model.
...
- fixed.prognostic:
  ...
  a parameters that describes weather the user fix some prognostic
  variable in the model (TRUE) or not (FALSE)
