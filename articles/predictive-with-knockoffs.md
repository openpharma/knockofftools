...
([Sechidis et al. 2021](#ref-predknockoffpaper))
...
([Lipkovich et al. 2017](#ref-lipkovich2017tutorial)):
``` math
\begin{align}
\mathbb{E}(Y| {X} = \mathbf{x} ,T = t) = h(\mathbf{x}) + g(\mathbf{x})t. 
\end{align}
```
Prognostic variables are those that contribute to $`h(\mathbf{x})`$
(i.e., ‘main effects’), while predictive are the ones that contribute to
$`g(\mathbf{x})`$. The focus of this vignette will be on the predictive
variables. Here we will show how to use use two different knockoff
filters to discover predictive variables, while controlling type-I
error.
...
(CATE): $`\mathbb{E} \left[ Y(1) - Y(0)| {X} = \mathbf{x}\right]`$.
