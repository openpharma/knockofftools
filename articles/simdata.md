...
$X_{1},\ldots,X_{30}$, one continuous response $y_{g}$, one binary
response $y_{b}$ and one set of censored survival times $T_{c}$. Let’s
...
the linear predictor $\ell_{p} = X\beta$ under sparsity, where the first
...
specified with `a`. Here we generate $\ell_{p}$ that implies association
...
*Simulation of Gaussian response:* $Y_{g}$ is Gaussian with mean
$\mu = \ell_{p}$ and standard deviation $\sigma = 1$:

``` r
Yg <- lp + rnorm(N)
```

*Simulation of Bernoulli response:* $Y_{b}$ is Bernoulli with success
probability
$\mu = exp\left( \ell_{p} \right)/\left( 1 + exp\left( \ell_{p} \right) \right)$:
...
generates censored survival times $T_{c}$ from a Cox regression model
$$\lambda(t) = \lambda_{0}(t)\exp\left( \ell_{p} \right),$$ with Weibull
baseline hazard: $$\lambda_{0}(t) = \lambda_{0}\rho t^{\rho - 1}$$ where
...
and $C$ have been simulated the function returns a survival object
(`Surv`) with `time = min(T, C)` and `event = 1{T < C}`:
